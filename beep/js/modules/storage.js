...
function migratePatternFields(pattern) {
    // Delete legcay swingEnabled (moved to global)
    ...
    try {
        ...
        if (saved) {
            ...
            if (state.swingLevel === undefined) {
                // Migrate from pattern-level swingEnabled if it exists
                const hasSwing = state.patterns.some(p => p.swingEnabled);
                state.swingLevel = hasSwing ? 'LIGHT' : 'OFF';
            }
