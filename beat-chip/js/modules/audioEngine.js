...
export class AudioEngine {
    constructor() {
        ...
        this.djDelay = null;
        this.djCrusher = null;
        this.djFolder = null;  // WaveFolder for harsh digital "Crush"
        this.djCrushGain = null; // Boost into folder
    ...
    async init() {
        ...
        // 1. BitCrusher + WaveFolder + Gain
        // We use a multi-stage approach for maximum "Crush"
        this.djCrusher = new Tone.BitCrusher({ bits: 2, wet: 0 });
        this.djCrushGain = new Tone.Gain(1);
        // Start with linear curve (bypass)
        ...
        // Connect chain: HPF → LPF → Delay → CrushGain → Folder → Crusher → Gate → Compressor → Limiter
        ...
        for (let i = 0; i < TRACKS.length; i++) {
            ...
            switch (track.type) {
                ...
                case 'chip-noise':
                    instrument = new Tone.NoiseSynth({
                        noise: { type: 'white' }, // BitCrush down the line handles the "chip" feel
                ...
                case 'pulse':
                    instrument = new Tone.PolySynth(Tone.Synth, {
                        maxPolyphony: 4,
                        oscillator: { type: 'pulse', width: 0.5 }, // Square wave
    ...
    triggerNote(track, pitch, duration, time, rollMode = false, rollSubdivision = 1, octaveShift = 0, velocity = 1.0) {
        ...
        if (velocity < 1.0) {
            if (trackConfig.type === 'noise' || trackConfig.type === 'chip-noise') {
                actualVelocity = 0.7;
            } else if (['fm', 'pulse', 'membrane', 'pulse-kick'].includes(trackConfig.type)) {
                actualVelocity = 0.5;
            } else {
                actualVelocity = 0.65;
            }
        ...
        for (let i = 0; i < triggers; i++) {
            ...
            if (trackConfig.type === 'membrane' || trackConfig.type === 'pulse-kick') {
            ...
            } else if (trackConfig.type === 'noise' || trackConfig.type === 'chip-noise') {
            ...
            } else if (trackConfig.type === 'fm' || trackConfig.type === 'pulse') {
                const totalPitch = pitch + (totalOctaveShift * 12);
                const freq = Tone.Frequency(trackConfig.baseFreq).transpose(totalPitch).toFrequency();
    ...
    updateTrackParams(track, params) {
        ...
        if (params.tune !== undefined) {
            ...
            if (trackConfig.type === 'membrane' || trackConfig.type === 'fm' || trackConfig.type === 'pulse-kick' || trackConfig.type === 'pulse') {
    ...
    // DJ CRUSH effect (Harsh Digital Folder + BitCrush)
    enableCrush() {
        if (this.djCrushEnabled || !this.initialized) return;
        this.djCrushEnabled = true;

        // Switch to fold curve and boost gain 8x
    ...
    disableCrush() {
        ...
        // Switch back to linear (transparent) curve and reset gain
        this.djFolder.curve = this.linearCurve;
        this.djCrushGain.gain.rampTo(1, 0.05);
        this.djCrusher.wet.rampTo(0, 0.05);
    }

    // WaveFold Curve: Folds values back instead of clipping
    _makeWaveFoldCurve() {
        const samples = 4096;
        const curve = new Float32Array(samples);
        for (let i = 0; i < samples; i++) {
            let x = (i / (samples - 1)) * 2 - 1; // -1 to 1
            // Triangle fold: folds the signal when it exceeds 0.25
            // Creating multiple "cycles" in the transfer function
